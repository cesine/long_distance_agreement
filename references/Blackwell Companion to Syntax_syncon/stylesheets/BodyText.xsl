...
<!--<xsl:call-template name="nest-apply-templates">
  ...
  <xsl:template name="fnote">
     <xsl:variable name="note" select="@rid"/>
      ...
      <xsl:if test="contains($note, ' fn')">
           <xsl:variable name="notes" select="substring-before($note, ' fn')"/>
           <xsl:value-of select="following::reference[@id = $notes]"/>
        </xsl:if>
  </xsl:template> 

<xsl:template name="bib">
     <xsl:variable name="ref" select="@rid"/>
       <xsl:value-of select="following::reference[@id = $ref]"/>
       <xsl:if test="contains($ref, ' b')">
           <xsl:variable name="refs" select="substring-before($ref, ' b')"/>
           <xsl:value-of select="following::reference[@id = $refs]"/>
        </xsl:if>
 </xsl:template>
...
<xsl:template match="link">
    <xsl:variable name="Item" select="@rid"/>
     ...
     <xsl:if test="not(contains(@rid, ' fn'))">
	  <xsl:if test="contains(@rid,'fn')">          
          	<a title="" href="javascript:makewindow('{@rid}')"><xsl:attribute name="title"><xsl:call-template name="fnote"/></xsl:attribute><xsl:apply-templates/></a>
       </xsl:if>
     </xsl:if>

      <xsl:if test="contains(@rid, ' fn')">
       <xsl:variable name="notes" select="substring-before(@rid, ' fn')"/>
	   	<a title="" href="javascript:makewindow('{$notes}')"><xsl:attribute name="title"><xsl:call-template name="fnote"/></xsl:attribute><xsl:apply-templates/></a>
      </xsl:if>


       <xsl:if test="not(contains(@rid, ' b'))">
          <xsl:if test="contains(@rid,'b')">
           <a title="" href="javascript:makewindow('{@rid}')"><xsl:attribute name="title"><xsl:call-template name="bib"/></xsl:attribute><xsl:apply-templates/></a>
          </xsl:if>
       </xsl:if>

	<xsl:if test="contains(@rid, ' b')">
           <xsl:variable name="refs" select="substring-before(@rid, ' b')"/>
           <a title="" href="javascript:makewindow('{$refs}')"><xsl:attribute name="title"><xsl:call-template name="bib"/></xsl:attribute><xsl:apply-templates/></a>
      </xsl:if>

	<xsl:if test="contains(@rid, ' t')">
        <xsl:variable name="tab" select="substring-before(@rid, ' t')"/>
        <a href="#{$tab}"><xsl:apply-templates/></a>
      </xsl:if>

	<xsl:if test="not(contains(@rid, ' t'))">
	 <xsl:if test="contains(@rid, 't')">
           <a href="#{$Item}"><xsl:apply-templates/></a>
      </xsl:if>
      </xsl:if>
...
function makewindow(url) 
{
  ...
  var ids=loc.indexOf('#');
  var path =  loc.substr(0,ids);
  var id = path + "#" + url;
  var a =  window.open(id,"popup","scrollbars,resizable,height=200,width=980,left=0,top=100,screenX=0,screenY=100");
  return;
}
</script>
</xsl:template>
